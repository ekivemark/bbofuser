...
from apps.bluebutton.file_def_cms import SEG_DEF
from apps.bluebutton.usa_states import STATES
...
def custom_family_history(strt_ln, ln_control, match_ln, strt_lvl,
    ...
    if DBUG:
        do_DBUG("pre-load the data passed in to ", seg_type,
                "<<<<<<<<<<<<<<<<",
    ...
    while not end_segment and (wrk_ln <= len(ln_list)-1):
        ...
        if DBUG:
            do_DBUG("Current_line:", current_line,
                    "match_ln:", match_ln,
                    "kvs:", kvs,
...
def write_conditions(wrk_ln, kvs, wrk_seg_def, ln_list):
    ...
    if DBUG:
        do_DBUG("END OF WRITE_CONDITIONS",
                "wrk_ln:", wrk_ln,
                "kvs:", kvs,
                "sub_kvs:", sub_kvs,
                "process_dict:", process_dict)
